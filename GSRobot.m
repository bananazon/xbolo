    ...
    if(!robots)
    {
        ...
        while((name = [enumerator nextObject]))
        {
            ...
            if([fullPath hasSuffix: @".xbolorobot"])
            {
                ...
                if([bundle load])
                {
                    // ARC manages memory; no autorelease needed
...
- (void)_watcherThread
{
    while(1)
    {
        @autoreleasepool {
            ...
            // We pass a CF-style free function and retained context so the robot can free when done.
            // Retain the objects array for the freeContext; it will be released by freeFunction.
            NSArray *objectsToDestroy = [[NSArray alloc] initWithObjects: gsdata, messages, nil];
            struct GSRobotCommandState commandState =
            [_robot stepXBoloRobotWithGameState:(void *)[gsdata bytes]
                                   freeFunction:(void (*)(void *))CFRelease
                                   freeContext:(__bridge_retained void *)objectsToDestroy];

            // Local strong references can be cleared; ownership has been transferred via __bridge_retained.
            // messages was created via copy; it’s retained in objectsToDestroy.

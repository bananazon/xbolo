...
- (IBAction)hostUPnPSwitch:(id)sender {
  BOOL aBool;
  aBool = [(NSButton *)sender state] == NSOnState;
  ...
  action = [menuItem action];
    ...
    default:
      return NO;
		...
		[(NSMenuItem *)menuItem setState:[menuItem tag] == builderToolInt ? NSOnState : NSOffState];
		return YES;
	}
    ...
    if (server.setup) {
      [(NSMenuItem *)menuItem setState:server.pause ? NSOnState: NSOffState];
      unlockserver();
      return YES;
    }
    else {
    ...
    if (server.setup) {
      [(NSMenuItem *)menuItem setState:server.allowjoin ? NSOnState: NSOffState];
      unlockserver();
      return YES;
    }
    else {
    ...
    [(NSMenuItem *)menuItem setState:muteBool];
    return YES;
  }
  ...
  if (!muteBool) {
    ...
    default:
      ...
      for (i = 0; i < [array count]; i++) {
        if (![(NSSound *)[array objectAtIndex:i] isPlaying]) {
          [(NSSound *)[array objectAtIndex:i] play];
          break;
        }
      }
    }

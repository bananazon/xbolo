...
+ (void)refresh {
  [NSAnimationContext runAnimationGroup:^(NSAnimationContext * _Nonnull context) {
    // We are not animating; we just want atomic coalesced updates.
    context.allowsImplicitAnimation = NO;

    NSEnumerator *enumerator = [boloViews objectEnumerator];
    GSBoloView *view = nil;
    while ((view = [enumerator nextObject]) != nil) {
      // Ask AppKit to redraw; -drawRect: will perform erase/refresh/draw.
      // We can mark the specific dirty rects if desired; here we just mark the view.
      [view setNeedsDisplay:YES];
    }
  } completionHandler:^{
    // No explicit flush calls; AppKit will commit updates at the end of the runloop.
  }];
...
TRY
  ...
  {
    NSPoint aPoint;
    // Convert the current mouse location (screen coords) to window coords, then to view coords.
    NSPoint screenPoint = [NSEvent mouseLocation];
    NSPoint windowPoint = [[self window] convertPointFromScreen:screenPoint];
    aPoint = [self convertPoint:windowPoint fromView:nil];
...
- (BOOL)becomeFirstResponder {
  ...
  if ((okToChange = [super becomeFirstResponder])) {
    modifiers = [NSEvent modifierFlags] & (NSEventModifierFlagCapsLock | NSEventModifierFlagShift | NSEventModifierFlagControl | NSEventModifierFlagOption | NSEventModifierFlagCommand | NSEventModifierFlagNumericPad | NSEventModifierFlagHelp | NSEventModifierFlagFunction);
  }
  return okToChange;
}
